<template>
  <div class="max-w-md m-auto py-10">
    ...
    <form @submit.prevent="addArtist">
      ...
      <input type="submit" value="Add Artist" class="font-sans font-bold px-4 rounded cursor-pointer no-underline bg-green hover:bg-green-dark block w-full py-4 text-white item-center justify-center"/>
    </form>

    <hr class="border border-gray-light my-6">
      ...
      <li class="py-4" v-for="artist in artists" :key="artist.id" :artist="artist">
        ...
        <div v-if="artist == editedArtist">
          <form @submit.prevent="updateArtist(artist)">
            <div class="mb-6 p-4 bg-white rounded border border-gray-light mt-4">
  ...
  methods: {
    ...
    removeArtist (artist) {
      this.$http.secured.delete(`/api/v1/artists/${artist.id}`)
        ...
        .catch(error => this.setError(error, 'Cannot delete artists'))
    },
    editArtist (artist) {
